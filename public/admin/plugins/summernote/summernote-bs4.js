...
var WrappedRange = /*#__PURE__*/function () {
  ...
  range_createClass(WrappedRange, [{
    ...
    /**
     * create offsetPath componentmark
     *
     * @param {Node} editable
     */
    ...
    key: "componentmark",
    value: function componentmark(editable) {
      return {
        s: {
    ...
    /**
     * create offsetPath componentmark base on paragraph
     *
     * @param {Node[]} paras
     */

  }, {
    key: "paracomponentmark",
    value: function paracomponentmark(paras) {
      return {
        s: {
...
/* harmony default export */ const range = ({
   ...
   * @method
   *
   * create WrappedRange from componentmark
   *
   * @param {Node} editable
   * @param {Object} componentmark
   * @return {WrappedRange}
   */
  createFromcomponentmark: function createFromcomponentmark(editable, componentmark) {
    var sc = dom.fromOffsetPath(editable, componentmark.s.path);
    var so = componentmark.s.offset;
    var ec = dom.fromOffsetPath(editable, componentmark.e.path);
    var eo = componentmark.e.offset;
    return new WrappedRange(sc, so, ec, eo);
  },
   ...
   * @method
   *
   * create WrappedRange from paracomponentmark
   *
   * @param {Object} componentmark
   * @param {Node[]} paras
   * @return {WrappedRange}
   */
  createFromParacomponentmark: function createFromParacomponentmark(componentmark, paras) {
    var so = componentmark.s.offset;
    var eo = componentmark.e.offset;
    var sc = dom.fromOffsetPath(lists.head(paras), componentmark.s.path);
    var ec = dom.fromOffsetPath(lists.last(paras), componentmark.e.path);
    return new WrappedRange(sc, so, ec, eo);
  }
});
  ...
  History_createClass(History, [{
    ...
    value: function makeSnapshot() {
      ...
      return {
        ...
        componentmark: rng && rng.isOnEditable() ? rng.componentmark(this.editable) : emptycomponentmark
      };
    }
  }, {
    key: "applySnapshot",
    value: function applySnapshot(snapshot) {
      ...
      if (snapshot.componentmark !== null) {
        range.createFromcomponentmark(this.editable, snapshot.componentmark).select();
      }
    }
    /**
    * @method rewind
...
var Bullet = /*#__PURE__*/function () {
  ...
  Bullet_createClass(Bullet, [{
    ...
    value: function toggleList(listName, editable) {
      ...
      var componentmark = rng.paracomponentmark(paras);
      ...
      range.createFromParacomponentmark(componentmark, paras).select();
    }
    /**
     * @param {Node[]} paras
...
var VideoDialog = /*#__PURE__*/function () {
  ...
  VideoDialog_createClass(VideoDialog, [{
    ...
    value: function createVideoNode(url) {
      ...
      var fbRegExp = /(?:www\.|\/\/)facecomponent\.com\/([^\/]+)\/videos\/([0-9]+)/;
      ...
      if (ytMatch && ytMatch[1].length === 11) {
        ...
        $video = external_jQuery_default()('<iframe>').attr('frameborder', 0).attr('src', 'https://www.facecomponent.com/plugins/video.php?href=' + encodeURIComponent(fbMatch[0]) + '&show_text=0&width=560').attr('width', '560').attr('height', '301').attr('scrolling', 'no').attr('allowtransparency', 'true');
      } else {
...
(external_jQuery_default()).summernote = external_jQuery_default().extend((external_jQuery_default()).summernote, {
  ...
  options: {
    ...
    codeviewIframeWhitelistSrcBase: ['www.youtube.com', 'www.youtube-nocookie.com', 'www.facecomponent.com', 'vine.co', 'instagram.com', 'player.vimeo.com', 'www.dailymotion.com', 'player.youku.com', 'jumpingbean.tv', 'v.qq.com'],
    keyMap: {
      pc: {
        'ESC': 'escape',
